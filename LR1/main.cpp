...
unordered_map<int, unordered_map<string, int>> gotoTable;  // {állapot} -> {nemterminális} -> {állapot}
...
bool parse(const string &input) {
    ...
    while (index <= input.length()) {  // Az index a bemenet végét is elérheti
        int state = stateStack.top();
        char currentChar = (index < input.length()) ? input[index] : '$';  // A bemeneti karakter

        // Ha van érvényes akció, végrehajtjuk
        if (actionTable[state].find(currentChar) != actionTable[state].end()) {
            ...
            if (action > 0) {
                ...
                cout << "Shift: " << currentChar << " -> state " << action << endl;
                index++;
            } else if (action < 0) {
                ...
                int newState = gotoTable[stateStack.top()][rule.left];
...
int main() {
    ...
    if (parse(input)) {
        cout << "The input is accepted by the LR(1) grammar." << endl;
    } else {
